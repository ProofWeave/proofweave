ProofWeave 토큰효율 벤치마크의 "답변 품질 비교"를 Gemini(너의 모델)로 수행해줘.
...
== 출력 ==
아래 JSON으로 회신하고 docs/evidence/benchmark-smoke-2026-06-01/quality-antigravity-<model>.json 에 저장(<model>=실제 사용 Gemini 모델 id). 각 쿼리의 raw/artifact 답변 원문도 answersText에 함께 남겨라(검증용).

{
  "evaluatorAgent": "antigravity",
  "model": "<실제 Gemini 모델 id>",
